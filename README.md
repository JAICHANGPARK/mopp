...
### 🚨 "가스! 가스! 가스!" — 알파(α) 긴급 경보 상황 대처법
작업 도중 예상치 못한 개인 정보 유출 위험이나 운영 DB 초기화 등 **비상 사태**를 감지하셨습니까? 망설이지 말고 즉시 터미널에 다음 경보를 외치십시오.
```bash
node core/bin/mopp gas
```
이 명령은 에이전트의 샌드박스를 즉시 `MOPP 4`로 차단하고 방독면과 보호장갑을 강제 장착(모든 변경 명령에 대한 수동 컨펌 강제)시킵니다.

위험 구역을 무사히 탈출한 후, 다시 평화로운 상태로 돌아가려면 경보를 해제해 주십시오.
```bash
