...
### 🚨 "GAS! GAS! GAS!" — Alpha Emergency Lockdown
Did you suddenly detect a critical credential leak or an accidental production database purge threat? Don't panic! Shout the emergency alarm:
```bash
node core/bin/mopp gas
```
This reactive command instantly locks down the agent's environment to `MOPP 4` (Maximum Containment, manual step-by-step confirmation required), effectively putting the mask and gloves on the agent.

Once the threat is neutralized and you've safely evacuated the hot zone, stand down and restore the normal dial:
```bash
